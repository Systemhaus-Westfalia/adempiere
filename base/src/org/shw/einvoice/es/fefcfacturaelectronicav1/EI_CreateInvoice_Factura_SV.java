...
public class EI_CreateInvoice_Factura_SV extends EI_CreateInvoice_Factura_SVAbstract
{
	...
	{	
		...
		if (invoice.getC_DocType().getE_DocType_ID()<= 0 ||
			...
			System.out.println("Process EI_CreateInvoice_Factura_SV : produced no errors");			
		}
		...
		FacturaFactory facturaBuilder = new FacturaFactory(get_TrxName(), getCtx(), client, orgInfo, invoice);
		facturaBuilder.generateJSONInputData(); // Will contain data passed to factory
		facturaBuilder.generateEDocument();
    	...
    	if (facturaBuilder.getEDocumentErrorMessages().length() > 0) {
    	...
    	String facturaAsJsonString = facturaBuilder.createJsonString();
       	invoiceElectronic.setjson(facturaAsJsonString);
    	invoiceElectronic.saveEx();
    	
    	if (isSaveInHistoric()) {
    		if (!facturaBuilder.writeToFile(facturaAsJsonString, invoice, Factura.ABSDIRECTORY)) {
